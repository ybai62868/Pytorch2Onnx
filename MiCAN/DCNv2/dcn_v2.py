...
class _DCNv2(Function):
    ...
    def symbolic(g, input, offset, mask, weight, bias,
        ...
        return g.op("DCN", input,
                    offset, mask,
                    weight, bias,
                    namespace_s = "",
                    name_s = "DCN", # plugin name
                    version_s = "v2", # plugin version
                    stride_i = stride[0],
                    padding_i = padding[0],
...
class DCN_sep_constrained(DCNv2):
    '''Use other features to generate offsets and masks, offsets have constrained range'''

    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, dilation=1,
                 deformable_groups=1, delta_y=None, delta_x=None):
        super(DCN_sep_constrained, self).__init__(in_channels, out_channels, kernel_size, stride, padding,
                                                  dilation, deformable_groups)
    ...
    def forward(self, input, fea):
        ...
        # add constrained mask
        if self.delta_y:
            lower_mask_y = torch.lt(o1, -self.delta_y).float()
            upper_mask_y = torch.gt(o1, self.delta_y).float()
            effective_mask_y = torch.ones_like(o1) - lower_mask_y - upper_mask_y
            o1 = effective_mask_y * o1 - self.delta_y * lower_mask_y + self.delta_y * upper_mask_y
        if self.delta_x:
            lower_mask_x = torch.lt(o2, -self.delta_x).float()
            upper_mask_x = torch.gt(o2, self.delta_x).float()
            effective_mask_x = torch.ones_like(o2) - lower_mask_x - upper_mask_x
            o2 = effective_mask_x * o2 - self.delta_x * lower_mask_x + self.delta_x * upper_mask_x
